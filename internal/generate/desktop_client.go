...
func (g *Generator) buildDesktopClientForm(_ Names) desktopClientFieldParts {
	...
	for _, f := range g.Definition.Fields {
		...
		case f.IsFile() || f.IsFiles():
			...
			if f.IsFiles() {
st.WriteString("  const [")
				st.WriteString(camel)
				st.WriteString(", ")
				st.WriteString(setter)
				st.WriteString("] = useState<FileRef[]>([]);\n")
pf.WriteString("      ")
				pf.WriteString(setter)
				pf.WriteString("((record.")
				pf.WriteString(json)
				pf.WriteString(" as FileRef[]) ?? []);\n")
jsx.WriteString("        <FileDropzone label=\"")
				jsx.WriteString(label)
				jsx.WriteString("\" value={")
				jsx.WriteString(camel)
				jsx.WriteString("} onChange={(v) => ")
				jsx.WriteString(setter)
				jsx.WriteString("((v as FileRef[]) ?? [])} multiple")
				jsx.WriteString(acceptAttr)
				jsx.WriteString(" />\n")
			} else {
st.WriteString("  const [")
				st.WriteString(camel)
				st.WriteString(", ")
				st.WriteString(setter)
				st.WriteString("] = useState<FileRef | null>(null);\n")
pf.WriteString("      ")
				pf.WriteString(setter)
				pf.WriteString("((record.")
				pf.WriteString(json)
				pf.WriteString(" as FileRef | null) ?? null);\n")
jsx.WriteString("        <FileDropzone label=\"")
				jsx.WriteString(label)
				jsx.WriteString("\" value={")
				jsx.WriteString(camel)
				jsx.WriteString("} onChange={(v) => ")
				jsx.WriteString(setter)
				jsx.WriteString("((v as FileRef) ?? null)}")
				jsx.WriteString(acceptAttr)
				jsx.WriteString(" />\n")
			}
pay.WriteString("      ")
			pay.WriteString(json)
			pay.WriteString(": ")
			pay.WriteString(camel)
			pay.WriteString(",\n")
...
imp.WriteString("import { ")
				imp.WriteString(hook)
				imp.WriteString(" } from \"@/hooks/use-")
				imp.WriteString(rel.PluralKebab)
				imp.WriteString("\";\n")
...
st.WriteString("  const [")
			st.WriteString(fkCamel)
			st.WriteString(", ")
			st.WriteString(fkSetter)
			st.WriteString("] = useState<string>(\"\");\n")
opt.WriteString("  const ")
			opt.WriteString(optsVar)
			opt.WriteString(" = ")
			opt.WriteString(hook)
			opt.WriteString("().data ?? [];\n")
pf.WriteString("      ")
			pf.WriteString(fkSetter)
			pf.WriteString("(String(record.")
			pf.WriteString(fk)
			pf.WriteString(" ?? \"\"));\n")
			jsx.WriteString("        <div>\n")
jsx.WriteString("          <label className=\"block text-[13px] font-medium text-foreground mb-1.5\">")
			jsx.WriteString(label)
			jsx.WriteString("</label>\n")
jsx.WriteString("          <SearchableSelect value={")
			jsx.WriteString(fkCamel)
			jsx.WriteString("} onChange={(v) => ")
			jsx.WriteString(fkSetter)
			jsx.WriteString("(v ?? \"\")} placeholder=\"Select ")
			jsx.WriteString(strings.ToLower(label))
			jsx.WriteString("…\"\n")
jsx.WriteString("            options={")
			jsx.WriteString(optsVar)
			jsx.WriteString(".map((o: any) => ({ value: String(o.id), label: String(o.name ?? o.title ?? o.id) }))} />\n")
			jsx.WriteString("        </div>\n")
pay.WriteString("      ")
			pay.WriteString(fk)
			pay.WriteString(": ")
			pay.WriteString(fkCamel)
			pay.WriteString(",\n")
...
st.WriteString("  const [")
			...
			st.WriteString(", ")
			st.WriteString(setter)
			st.WriteString("] = useState<number>(0);\n")
pf.WriteString("      ")
			pf.WriteString(setter)
			pf.WriteString("(Number(record.")
			pf.WriteString(json)
			pf.WriteString(" ?? 0));\n")
			jsx.WriteString("        <div>\n")
jsx.WriteString("          <label className=\"block text-[13px] font-medium text-foreground mb-1.5\">")
			jsx.WriteString(label)
			jsx.WriteString("</label>\n")
jsx.WriteString("          <NumberInput value={")
			jsx.WriteString(camel)
			jsx.WriteString("} onChange={")
			jsx.WriteString(setter)
			jsx.WriteString("} kind=\"")
			jsx.WriteString(kind)
			jsx.WriteString("\" className={inputCls} />\n")
			jsx.WriteString("        </div>\n")
pay.WriteString("      ")
			pay.WriteString(json)
			pay.WriteString(": ")
...
st.WriteString("  const [")
			...
			st.WriteString(", ")
			st.WriteString(setter)
			st.WriteString("] = useState<boolean>(false);\n")
pf.WriteString("      ")
			pf.WriteString(setter)
			pf.WriteString("(Boolean(record.")
			pf.WriteString(json)
			pf.WriteString("));\n")
			jsx.WriteString("        <label className=\"flex items-center gap-2 text-[13px] text-foreground\">\n")
jsx.WriteString("          <input type=\"checkbox\" checked={")
			jsx.WriteString(camel)
			jsx.WriteString("} onChange={(e) => ")
			jsx.WriteString(setter)
			jsx.WriteString("(e.target.checked)} className=\"h-4 w-4\" />\n")
jsx.WriteString("          ")
			jsx.WriteString(label)
			jsx.WriteString("\n")
			jsx.WriteString("        </label>\n")
...
pf.WriteString("      ")
			...
			pf.WriteString(" ? String(record.")
			pf.WriteString(json)
			pf.WriteString(").slice(0, ")
			pf.WriteString(slice)
			pf.WriteString(") : \"\");\n")
			jsx.WriteString("        <div>\n")
jsx.WriteString("          <label className=\"block text-[13px] font-medium text-foreground mb-1.5\">")
			jsx.WriteString(label)
			jsx.WriteString("</label>\n")
jsx.WriteString("          <input type=\"")
			jsx.WriteString(inputType)
			jsx.WriteString("\" value={")
			jsx.WriteString(camel)
			jsx.WriteString("} onChange={(e) => ")
			jsx.WriteString(setter)
			jsx.WriteString("(e.target.value)} className={inputCls} />\n")
			jsx.WriteString("        </div>\n")
pay.WriteString("      ")
			pay.WriteString(json)
			pay.WriteString(": ")
...
jsx.WriteString("          <label className=\"block text-[13px] font-medium text-foreground mb-1.5\">")
			jsx.WriteString(label)
			jsx.WriteString("</label>\n")
jsx.WriteString("          <textarea value={")
			jsx.WriteString(camel)
			jsx.WriteString("} onChange={(e) => ")
			jsx.WriteString(setter)
			jsx.WriteString("(e.target.value)} rows={4} className={inputCls} />\n")
			jsx.WriteString("        </div>\n")
pay.WriteString("      ")
			pay.WriteString(json)
			pay.WriteString(": ")
...
jsx.WriteString("          <label className=\"block text-[13px] font-medium text-foreground mb-1.5\">")
			jsx.WriteString(label)
			jsx.WriteString("</label>\n")
jsx.WriteString("          <input type=\"text\" value={")
			jsx.WriteString(camel)
			jsx.WriteString("} onChange={(e) => ")
			jsx.WriteString(setter)
			jsx.WriteString("(e.target.value)} className={inputCls} />\n")
